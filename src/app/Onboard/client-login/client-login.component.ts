    ...
    if(userdetail.valid){
      ...
      this.commonservice.PostApi(detail,url).subscribe(((response : any) =>{
        if(response.status == 1){
          ...
          localStorage.setItem('sw_MemberDetail', <any>JSON.stringify(response.data.family_member));
          ...
          if (localStorage.getItem('web_view_id')) {
            this.route.navigate(['/consent-form'], { queryParams: { id: localStorage.getItem('web_view_id') } });
          } else {
            if(response.data.address){ 
              if(response.data.consent_status < 7){
              ...
              if(!response.data.subscription.isPurchase){
                localStorage.setItem('sw_subscription',<any> JSON.stringify(response.data.subscription));
              }       
            } else{
